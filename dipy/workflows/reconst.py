...
from dipy.reconst.dti import mode as get_mode
...
class ReconstMAPMRIFlow(Workflow):
    ...
    def run(
        ...
        for (
            ...
            logging.info(
                "MAPMRI saved in {0}".format(os.path.dirname(out_dir))
            )
...
class ReconstDtiFlow(Workflow):
    ...
    def run(
        ...
        bvalues_files: Union[str, list[str]],
        bvectors_files: Union[str, list[str]],
        mask_files: Union[str, list[str]],
        fit_method: str = "WLS",
        b0_threshold: Union[int, float] = 50,
        bvecs_tol: float = 0.01,
        sigma: np.ndarray = None,
        save_metrics: list[str] = [],
        out_dir: str = "",
        out_tensor: str = "tensors.nii.gz",
        out_fa: str = "fa.nii.gz",
        out_ga: str = "ga.nii.gz",
        out_rgb: str = "rgb.nii.gz",
        out_md: str = "md.nii.gz",
        out_ad: str = "ad.nii.gz",
        out_rd: str = "rd.nii.gz",
        out_mode: str = "mode.nii.gz",
        out_evec: str = "evecs.nii.gz",
        out_eval: str = "evals.nii.gz",
        nifti_tensor: bool = True,
    ):
        ...
        for (
            ...
            tenfit, _ = self.get_fitted_tensor(
                data,
                mask,
                bval,
                bvec,
    ...
    def estimate_default_sigma(self, data: np.ndarray):
        """
        By default, estimate a reasonable amount of noise in *data* using *dipy.denoise.noise_estimate.estimate_sigma* function

        Parameters
        ----------
        data : np.ndarray
            An array of pixels from the diffusion data

        Returns
        -------
        np.ndarray
            A reasonable estimate for amount of noise in *data*
        """
...
class ReconstCSDFlow(Workflow):
    ...
    @deprecated_params(
        "nbr_processes", "num_processes", since="1.4", until="1.5"
    )
    def run(
        self,
        input_files,
        ...
        for (
            ...
            logging.info(
                "Eigenvalues for the frf of the input"
    ...
    def run(
        ...
        for (
            ...
            if "dt_tensor" in save_metrics:
                ...
                save_nifti(
                    otensor, tensor_vals_reordered.astype(np.float32), affine
                )
            ...
            logging.info(
                "DKI metrics saved in {0}".format(os.path.dirname(oevals))
            )
    ...
    def run(
        ...
        for (
            ...
            ivimfit, _ = self.get_fitted_ivim(
                data, mask, bval, bvec, b0_threshold
            )
            ...
            if "S0_predicted" in save_metrics:
                save_nifti(
                    oS0_predicted,
                    ivimfit.S0_predicted.astype(np.float32),
                    affine,
                )
            ...
            logging.info(
                "IVIM metrics saved in {0}".format(os.path.dirname(oD))
            )
