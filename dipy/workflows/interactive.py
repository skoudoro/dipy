...
def interactive_preprocessing_selection():
    ...
    print("\nEnter numbers (comma-separated, e.g., 1,2,5) or press Enter for all:")
    choice = input("Steps: ").strip()

    selected = {}
    if not choice:
        for step_name in preprocessing_steps:
            selected[step_name] = True
    else:
        try:
            ...
            print("Invalid input, selecting all steps")
            for step_name in preprocessing_steps:
                selected[step_name] = True
...
def build_interactive_pipeline_config(*, data_chars):
    ...
    if preprocessing.get("denoise", False):
        ...
        current_input = "${denoise.out_denoised}"
