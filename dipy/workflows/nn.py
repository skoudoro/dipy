...
class BiasFieldCorrectionFlow(Workflow):
    ...
    def run(
        ...
        verbose=False,
        out_dir="",
        out_corrected="dwi_corrected.nii.gz",
    ):
        """Correct bias field.
        ...
        method : string, optional
            Bias field correction method. Choose from:
                - 'n4': DeepN4 bias field correction (for T1w images).
                  See :footcite:p:`Kanakaraj2024` for more details.
                - 'b0': B0 bias field correction via normalization (For DWI images).
        ...
        if method.lower() not in ["n4", "b0"]:
            logging.error("Unknown bias field correction method. Choose from 'n4, '.")
            sys.exit(1)
