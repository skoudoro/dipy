...
def test_io_peaks():
    with InTemporaryDirectory():
        ...
        pam2_res = load_pam(fname2, verbose=False)
        ...
        npt.assert_raises(ValueError, save_pam, fname3, pam4)
...
def test_io_save_pam_error():
    with InTemporaryDirectory():
        ...
        npt.assert_raises(IOError, save_pam, 'test.pam', pam)
        npt.assert_raises(ValueError, save_pam, fname, pam)
...
def test_tensor_to_pam():
    ...
    with InTemporaryDirectory():
        ...
        pam = tensor_to_pam(evals=df.evals, evecs=df.evecs, affine=affine,
                            sphere=sphere, odf=odf, pam_file=fname)
        npt.assert_(os.path.isfile(fname))
        save_pam('test_tt_2.pam5', pam)
